...
async function clearQueue() {
  ...
  try {
    ...
    if (stats.waiting > 0) {
      console.log(`Found ${stats.waiting} waiting jobs. Clearing...`);
    ...
    if (stats.active > 0) {
      console.log(`Found ${stats.active} active jobs. Clearing...`);
      await smartClipperQueue.clean(0, 'active');
      console.log('Cleared all active jobs');
    }
    
    if (stats.completed > 0) {
      console.log(`Found ${stats.completed} completed jobs. Clearing...`);
      await smartClipperQueue.clean(0, 'completed');
      console.log('Cleared all completed jobs');
    }
    
    if (stats.failed > 0) {
      console.log(`Found ${stats.failed} failed jobs. Clearing...`);
