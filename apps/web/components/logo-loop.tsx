...
const useAnimationLoop = (
  ...
  useEffect(() => {
    ...
    // Initialize offset smoothly without modulo reset
    if (seqSize > 0 && offsetRef.current === 0) {
    ...
    const animate = (timestamp: number) => {
      ...
      if (seqSize > 0) {
        ...
        // Only reset when we fully pass one sequence length (seamless infinite loop)
        if (nextOffset >= seqSize) {
          nextOffset -= seqSize;
        } else if (nextOffset <= -seqSize) {
          nextOffset += seqSize;
        }
